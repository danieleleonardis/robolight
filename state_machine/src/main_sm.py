    ...
    def execute(self, userdata):
        ...
        cnt=0
       	while state_decider() == 'supervised' and not rospy.is_shutdown():
            output_twist = limitTwist(supervised_twist)
            if cnt<10:
                udp_message.write_motor=True
            else:
                udp_message.write_motor=False
...
pub_udpcmd = rospy.Publisher('cmd_vel_udp', UDPmessage, queue_size=5)
pub_cmdvel = rospy.Publisher('cmd_vel', Twist, queue_size=5)
